    ...
    <main className={`showroom-page ${cinema ? "cinema-mode" : ""}`} style={{ "--accent": dealership.accent, "--accent2": dealership.accent2 }}>
      <section className="showroom-hero" ref={videoRef}>
        ...
        <div className="desktop-panel">
          <div className="glass-card inspect-card">
            <h2>Inspect Vehicle</h2>
            <p>Choose a section and the showroom video switches straight to it.</p>
            <InspectCards car={car} activeLabel={activeLabel} onSelect={selectClip} variant="compact" />
          </div>
      ...
      <AnimatePresence>
        {inspectPopup && (
          <motion.div className={`inspect-popup-backdrop ${cinema ? "cinema-inspect" : ""}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
